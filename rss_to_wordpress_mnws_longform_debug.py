...
for entry in feed.entries:
    print("• Artikel:", entry.title)
    if entry.link in seen_articles:
        print("⏭️  Overgeslagen (reeds gepost):", entry.link)
        continue
...
if __name__ == "__main__":
    ...
    with_images = sys.argv[3] if len(sys.argv) > 3 else "yes"
    longform = sys.argv[4] if len(sys.argv) > 4 else "long"
    selected_date = datetime.strptime(date_input, "%Y-%m-%d").date()
    feeds = feed_string.split(";;") if feed_string else DEFAULT_FEEDS
    if longform == "short":
        def translate_text(text, style="article"):
            if style == "title":
                prompt = "Vertaal deze titel naar helder en correct Nederlands, zonder aanhalingstekens of uitleg."
            else:
                prompt = (
                    "Vat dit artikel samen in het Nederlands in een neutrale, informatieve stijl zoals op MNWS.be. "
                    "Gebruik een korte, heldere schrijfstijl, zonder bronvermelding."
                )
            try:
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo",
    ...
    def post_to_wordpress(title, content, image_id=None):
        ...
        if with_images == "yes" and image_id:
            data["featured_media"] = image_id
        response = requests.post(endpoint, json=data, auth=(WORDPRESS_USER, WORDPRESS_APP_PASS))
        if response.status_code == 201:
            print("✅ Gepost:", title)
